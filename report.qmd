...
features <-  c("Cnr1", "Cnr2", "Nr3c1", "Nr3c2", "Dagla", "Daglb", "Mgll", "Faah", "Napepld")

# Generate markers
GSE255405.SVZ.markers <- subset(GSE255405, subset = area == "SVZ"  & time == "P7") %>% FindAllMarkers(., test.use = "MAST", features = features)
GSE255405.SVZ.markers <- GSE255405.SVZ.markers %>% filter(avg_log2FC > 0)
GSE255405.SVZ.markers <- GSE255405.SVZ.markers %>%
  mutate(comparison = "All_V/SVZ", group = "MainGroups")

GSE255405.ctx.markers <- subset(GSE255405, subset = area == "cortex" & time == "P7") %>% FindAllMarkers(., test.use = "MAST", features = features)
GSE255405.ctx.markers <- GSE255405.ctx.markers %>% filter(avg_log2FC > 0)
GSE255405.ctx.markers <- GSE255405.ctx.markers %>%
  mutate(comparison = "All_Cortex", group = "MainGroups")

Idents(GSE255405_OL) <- "MinorGroups"
...
GSE255405_OL.SVZ.markers <- GSE255405_OL.SVZ.markers %>%
  mutate(comparison = "All_V/SVZ", group = "MinorGroups")

GSE255405_OL.CTX.markers <- subset(GSE255405_OL, subset = area == "cortex"  & time == "P7") %>% FindAllMarkers(., test.use = "MAST", features = features)
GSE255405_OL.CTX.markers <- GSE255405_OL.CTX.markers %>% filter(avg_log2FC > 0)
GSE255405_OL.CTX.markers <- GSE255405_OL.CTX.markers %>%
  mutate(comparison = "All_Cortex", group = "MinorGroups")


GSE255405.markers <- rbind(GSE255405.SVZ.markers, GSE255405.ctx.markers, GSE255405_OL.SVZ.markers, GSE255405_OL.CTX.markers)
...
markers_area_OL$group = "MinorGroups"
...
#   }
#   tab <- GSE %>%
#     FindMarkers(., ident.1 = "SVZ", ident.2 = "cortex", test.use = "negbinom", group.by = "area", features = features)
#   tab$cluster = cell
#   tab$gene = rownames(tab)
#   return(tab)
# }) %>% Reduce(rbind, .)
...
# 
# markers_OL_sub.SVZ.markers <- subset(GSE255405_OL, subset = area == "SVZ"  & time == "P7") %>% FindAllMarkers(., test.use = "MAST", features = features)
# markers_OL_sub.SVZ.markers <- markers_OL_sub.SVZ.markers %>% filter(avg_log2FC > 0)
# markers_OL_sub.SVZ.markers <- markers_OL_sub.SVZ.markers %>% mutate(comparison = "All_V/SVZ")
# 
# markers_OL_sub.CTX.markers <- subset(GSE255405_OL, subset = area == "cortex"  & time == "P7") %>% FindAllMarkers(., test.use = "MAST", features = features)
# markers_OL_sub.CTX.markers <- markers_OL_sub.CTX.markers %>% filter(avg_log2FC > 0)
# markers_OL_sub.CTX.markers <- markers_OL_sub.CTX.markers %>% mutate(comparison = "All_Cortex")

markers <- rbind(GSE255405.markers, markers_area, markers_area_OL)
...
valid_main_area <- GSE255405@meta.data %>%
  group_by(MainGroups, area) %>%
  tally(name = "n") %>%
  filter(n >= 10) %>%
  select(MainGroups, area)

meta_main <- GSE255405@meta.data
meta_main$cell <- rownames(meta_main)
cells_keep_main <- meta_main %>%
  semi_join(valid_main_area, by = c("MainGroups", "area")) %>%
  pull(cell)
GSE255405 <- subset(GSE255405, cells = cells_keep_main)

valid_minor_area <- GSE255405_OL@meta.data %>%
  group_by(MinorGroups, area) %>%
  tally(name = "n") %>%
  filter(n >= 10) %>%
  select(MinorGroups, area)

meta_minor <- GSE255405_OL@meta.data
meta_minor$cell <- rownames(meta_minor)
cells_keep_minor <- meta_minor %>%
  semi_join(valid_minor_area, by = c("MinorGroups", "area")) %>%
  pull(cell)
GSE255405_OL <- subset(GSE255405_OL, cells = cells_keep_minor)

markers <- readRDS("All_comparisons.p7.markers.rds")

glu_genes <- c("Cnr1", "Cnr2", "Nr3c1", "Nr3c2", "Dagla", "Daglb", "Mgll", "Faah", "Napepld")
...
p <- subset(GSE255405, subset = time == "P7" & area == "SVZ") %>%
...
stats <- markers %>% filter(group == "MainGroups",
                            comparison %in% c("All_V/SVZ", "All_Cortex"),
                            p_val_adj < 0.05)
...
# Extract data from dotplots
d2 <- p2$data
d3 <- p3$data

# Add asterisk column
d2 <- merge(d2, stats %>% filter(comparison == "All_V/SVZ"), by.x = c("id","features.plot"), by.y = c("cluster","gene"), all = T) %>% mutate(asterisk = ifelse(p_val_adj < 0.05, "*",""))

d3 <- merge(d3, stats %>% filter(comparison == "All_Cortex"), by.x = c("id","features.plot"), by.y = c("cluster","gene"), all = T) %>% mutate(asterisk = ifelse(p_val_adj < 0.05, "*",""))

d2 <- d2 %>% filter(!is.nan(avg.exp.scaled)) %>% mutate(asterisk = ifelse(is.na(asterisk), "", asterisk))
d3 <- d3 %>% filter(!is.nan(avg.exp.scaled)) %>% mutate(asterisk = ifelse(is.na(asterisk), "", asterisk))

# Rebuild plots with asterisk annotation
library(ggplot2)
p2_mod <- p2 + geom_text(data = d2[d2$asterisk == "*", ], aes(label = asterisk), vjust = 0.2, size = 7)
p3_mod <- p3 + geom_text(data = d3[d3$asterisk == "*", ], aes(label = asterisk), vjust = 0.2, size = 7)
...
**Figure 3.**\
Comparative dot plots illustrating the expression of glucocorticoid and cannabinoid signaling--related genes (**`r paste(glu_genes)`**) in different brain regions and developmental stages.\
...
p <- subset(GSE255405_OL, time == "P7" & area == "SVZ") %>%
  DimPlot(., group.by = "area", label = T, pt.size = 1) |
  subset(GSE255405_OL, time == "P7" & area == "SVZ") %>% DimPlot(., group.by = "MainGroups", pt.size = 1) |
  subset(GSE255405_OL, time == "P7" & area == "SVZ") %>% DimPlot(., group.by = "MinorGroups", pt.size = 1)
...
stats <- markers %>% filter(group == "MinorGroups",
                            comparison %in% c("All_V/SVZ", "All_Cortex"),
                            p_val_adj < 0.05)
...
heatmap_base <- markers %>%
  filter(comparison == "VSVZ_vs_Cortex") %>%
  select(cluster, gene, p_val_adj, avg_log2FC) %>%
  distinct(cluster, gene, .keep_all = TRUE)

all_clusters <- sort(unique(heatmap_base$cluster))
all_genes <- sort(unique(heatmap_base$gene))

heatmap_data <- tidyr::expand_grid(cluster = all_clusters, gene = all_genes) %>%
  left_join(heatmap_base, by = c("cluster", "gene")) %>%
  mutate(
    signif_label = ifelse(!is.na(p_val_adj) & p_val_adj < 0.05, "*", ""),
    gene = factor(gene, levels = all_genes),
    cluster = factor(cluster, levels = all_clusters)
  )

ggplot(heatmap_data, aes(x = gene, y = cluster, fill = avg_log2FC)) +
  geom_tile(color = "black", linewidth = 0.2) +
  geom_text(aes(label = signif_label), size = 5) +
  ...
  labs(
    title = "VSVZ vs Cortex comparison",
    x = "",
    y = ""
  ) +
...
**Figure 6.**\
Heatmap of differential expression for the VSVZ vs Cortex comparison only in MainGroups. Color indicates `avg_log2FC` and asterisks (\*) mark genes with adjusted p-value \< 0.05.
...
Mouse data - Zhang et al. (2014) An RNA-sequencing transcriptome and splicing database of glia, neurons, and vascular cells of the cerebral cortex. J Neurosci 34(36):11929-11947. PMID: 25186741.
...
# Mus_musculus - https://brainrnaseq.org/wp-content/uploads/2022/09/fe-wp-dataset-120.csv
mus_barnes <- read.csv("barnes/fe-wp-dataset-120.csv")
mus_barnes <- mus_barnes %>% filter(grepl(paste(glu_genes, collapse = "|"), gene_id)) %>%
  mutate(gene_id = sub(" - .*", "", gene_id)) %>% select(-2)

mus_barnes <- mus_barnes %>%
  # Step 1: pivot to long format first
  pivot_longer(
    cols = -gene_id,
    names_to = "cell_type",
    values_to = "value"
  ) %>%
  # Step 2: remove replicate suffix (_1, _2, etc.)
  mutate(cell_type = sub("_[0-9]+$", "", cell_type),
             # rename nicely
          cell_type = recode(cell_type,
            "astrocytes" = "Astrocytes",
            "neurons" = "Neurons",
            "opc" = "OPC",
            "newly_formed_oligodendrocyte" = "nfOligodendrocyte",
            "myelinating_oligodendrocyte" = "mOligodendrocyte",
            "endothelial" = "Endothelial",
            "microglia_macrophage" = "Microglia/Macrophage")) %>%
  # Step 3: compute median per gene + cell type
  group_by(gene_id, cell_type) %>%
  summarise(median_value = median(value, na.rm = TRUE), .groups = "drop")

ggplot(mus_barnes, aes(x = gene_id, y = cell_type, fill = log2(1+median_value))) +
  geom_tile(color = "black", linewidth = 0.2) +
  scale_fill_gradient2(
    low = "darkblue",
    mid = "white",
  ...
  labs(
    title = "Cortex P7 (Zhang et al, 2014)",
    x = "",
    y = ""
  ) +
  theme_minimal(base_size = 11) +
...
**Figure 7.**\
Heatmap of P7 mouse brain cortex major groups. Color indicates `log2(FPKM+1)` values. nfOligodendrocyte (Newly formed oligodendrocytes) and mOligodendrocyte (Myelinating Oligodendrocytes).
